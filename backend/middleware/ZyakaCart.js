...
const ZyakaCart = async function (req, res, next) {
  ...
  //   return res
  //     .status(400)
  //     .json({ error: 'You need to delete your Zyaka cart first ' });
  // next();
  if (!user.selectedRestaurant || user.selectedRestaurant === 'PunjabiZyaka')
    next();
  else
    res
      .status(400)
