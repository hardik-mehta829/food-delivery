...
const CartChecker = require('../middleware/DreamArenaCart');
...
Router.get('/', async (req, res) => {
  try {
    ...
    let query = DreamArena.find(JSON.parse(queryStr));
    if (req.query.sort) query = query.sort(req.query.sort);
    const page = req.query.page || 1;
    const skip = (page - 1) * 5;
    const numitems = await DreamArena.countDocuments();
    if (skip >= numitems)
...
Router.get('/item/:id', fetchuser, async (req, res) => {
  try {
    ...
    const data = await DreamArena.findOne({ _id: id }).populate({
      path: 'reviews',
...
Router.post('/', async (req, res) => {
  try {
    const data = await DreamArena.create(req.body);
    res.status(200).json(data);
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
...
Router.post('updateitem/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const data = await DreamArena.findOneAndUpdate({ _id: id }, req.body, {
      new: true,
      runValidators: true,
    });
    res.status(200).json(data);
...
Router.delete('deleteitem/:id', async (req, res) => {
  const id = req.params.id;
  const data = await DreamArena.deleteOne({ _id: id });
  res.status(200).json(data);
});
Router.get('/viewcart', fetchuser, CartChecker, async function (req, res) {
  try {
    ...
    if (user.cartDreamArena.length === 0)
      throw { statusCode: 400, message: 'This cart is empty' };

    res.status(200).json({
      results: user.cartDreamArena.length,
      cart: user.cartDreamArena,
...
Router.get('/additem/:id', fetchuser, CartChecker, async function (req, res) {
  try {
    const user = await usermodel.findOne({ _id: req.user._id });
    const cartItems = [...user.cartDreamArena];
    if (cartItems.length === 0) {
      user.selectedRestaurant = 'DreamArena';
    }
    ...
    if (x) {
      ...
      user.cartDreamArena = [...y];
      // x.quantity = x.quantity + 1;
      // user.cartPunjabiZyaka = [...y, x];
    } else {
      const data = await DreamArena.findOne({ _id: req.params.id });
    ...
    user.totalcost = user.cartDreamArena.reduce((total, item) => {
...
Router.post('/placeorder', fetchuser, CartChecker, async function (req, res) {
  try {
    ...
    const length = user.cartDreamArena.length;
    user.cartDreamArena = [];
